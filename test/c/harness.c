...
/* Forward + log every pending datagram. Direction by source port. */
static void proxy_pump(void) {
    ...
    for (;;) {
        ...
        if (ntohs(from.sin_port) == CLIENT_PORT) {
...
typedef enum { ROLE_C, ROLE_S } Role;
...
static void perform_action(const Action *act, ENetHost *client, ENetHost *server,
                           ENetPeer **client_peer, ENetPeer **server_peer,
                           int *client_stopped) {
    ENetPeer *peer = NULL;
    Role r = act->role;
    ...
    case ACT_CONNECT: {
        ...
        ENetPeer *p = enet_host_connect(client, &addr, act->a, act->b);
        if (p == NULL) { fprintf(stderr, "connect failed\n"); exit(1); }
        *client_peer = p;
        fprintf(trace_file, "A %llu C CONNECT host=127.0.0.1:%u channels=%u data=%u\n",
                (unsigned long long)now_ms(), PROXY_PORT, act->a, act->b);
        return;
    }
    case ACT_SEND: {
        peer = (r == ROLE_C) ? *client_peer : *server_peer;
        ...
        fprintf(trace_file, "A %llu %s SEND ch=%u flags=%u hex=%s\n",
                (unsigned long long)now_ms(), role_name(r), act->ch,
                act->flags, hex);
        return;
    }
    case ACT_DISCONNECT: {
...
static void run_scenario(const Scenario *sc) {
    ENetAddress client_addr_en, server_addr_en;
    ...
    while ((t = now_ms()) < sc->duration_ms) {
        for (size_t i = 0; i < sc->action_count; i++) {
            if (!done[i] && sc->actions[i].at_ms <= t) {
                perform_action(&sc->actions[i], client, server,
                               &client_peer, &server_peer, &client_stopped);
                done[i] = 1;
            }
        }
